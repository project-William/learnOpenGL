	...
	namespace graphics {
		...
		void Render::renderPipes() {
			...
			if (-delay <= 4000) {
				...
				for (int i = 0; i < 8; i += 2) {
					translate = math::Matrix4f::translate(math::Vec3(i / 2 * 0.5f + ref1 * 0.0005f, -0.9f + randomup[i] / 4, 0.0f));
					pipes[i]->setup(translate);
					pipes[i]->use();
				}

				for (int i = 1; i < 8; i += 2) {
					translate = math::Matrix4f::translate(math::Vec3(-3.9f + (i - 1) / 2 * 0.5f - ref1 * 0.0005f, -1.0f + randombottom[i] / 4, 0.0f));
					rotate = math::Matrix4f::rotate(180.0f);
					view = math::multiply(rotate, translate);
					pipes[i]->setup(view);
					pipes[i]->use();
