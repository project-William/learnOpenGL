...
void Shader::VaoVboEbo(GLint num) {
	glGenVertexArrays(num, VAOs);
	glGenBuffers(num, VBOs);
	glGenBuffers(num, EBOs);
}

void Shader::RectangularSetup(GLsizeiptr size, const void* data,GLsizeiptr ele_size,const void* ele_data) {
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, ele_size, ele_data, GL_STATIC_DRAW);
